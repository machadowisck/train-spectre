...
class Trainer(object):
    ...
    def prepare_training_losses(self):
        ...
        config = ConfigParser()
        print(self.cfg.model.lipread_config)
        config.read(self.cfg.model.lipread_config)
        config['model']['model_path']=osp.join(self.cfg.lrs3_modelpath,'model.pth')
        config['model']['model_conf'] = osp.join(self.cfg.lrs3_modelpath, 'model.json')
    ...
    def fit(self):
        ...
        if 'state_dict' in checkpoint.keys():
            processed_checkpoint = checkpoint['state_dict']
        else:
            processed_checkpoint = checkpoint
        self.spectre.E_flame.load_state_dict(processed_checkpoint['E_flame'],strict=False)#
        self.spectre.E_expression.load_state_dict(processed_checkpoint['E_expression'],strict=False)
        self.freeze_layer(self.spectre)
        normal_params = []
        for name, param in self.spectre.named_parameters():
            if param.requires_grad:
                print(f'需要訓練的參數：{name}')
                normal_params.append(param)

        # self.opt = torch.optim.Adam(self.spectre.E_expression.parameters(),self.cfg.train.lr)
        self.opt = torch.optim.Adam(normal_params,self.cfg.train.lr)

        scheduler = torch.optim.lr_scheduler.MultiStepLR(self.opt,[50000],gamma=0.2)
        iter_all=0
        # train_all_iter=self.cfg.earlystopping_step+10
        for epoch in range(start_epoch, self.cfg.train.max_epochs):
            self.epoch = epoch
            all_loss_mean = {}
            # if iter_all > train_all_iter:
            #     break
            for batch in self.train_dataloader:
                ...
                # if iter_all==train_all_iter:
                    ...
                    print(os.path.join(self.cfg.output_dir, 'spectreModel_singleFinetune.tar'))
                    torch.save(model_dict, os.path.join(self.cfg.output_dir, 'spectreModel_singleFinetune.tar'))
    ...
    def freeze_layer(self, network):
        for name, parms in network.named_parameters():
            if 'adapter' in name :  # 針對原始的網絡層進行freeze，對於新加入的網絡層進行學習
    ...
    def prepare_data(self):
        ...
        from datasets.our_datasets import get_datasets_OURS

        self.train_dataset, self.val_dataset, self.test_dataset = \
            get_datasets_OURS(self.cfg)
