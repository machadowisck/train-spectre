...
class Adapter_Conv(nn.Module):
    def __init__(self, in_channels, m_channels=16):
        ...
        self.conv1 = convlayer(in_channels, m_channels)
        self.conv2 = convlayer(m_channels, m_channels)
        self.conv3 = convlayer(m_channels, in_channels)
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000):
       ...
       # self.fc = nn.Linear(512 * block.expansion, num_classes)
        self.adatper=True
        if self.adatper:
            self.adapter_1=Adapter_Conv(64*block.expansion)
            # self.adapter_1_1=Adapter_Conv(64*block.expansion)
            self.adapter_2=Adapter_Conv(128*block.expansion)
            self.adapter_3 = Adapter_Conv(256 * block.expansion)
            # self.adapter_3_1 = Adapter_Conv(256 * block.expansion)

            self.adapter_4 = Adapter_Conv(1024)
